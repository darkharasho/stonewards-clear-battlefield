...
plugin = re.search(r'PluginVersion = "([^"]+)"', (root / "src/ClearBattlefield/Plugin.cs").read_text())
csproj = re.search(r"<Version>([^<]+)</Version>", (root / "src/ClearBattlefield/ClearBattlefield.csproj").read_text())
...
if not csproj or csproj.group(1) != v:
    errors.append(f"ClearBattlefield.csproj Version is {csproj and csproj.group(1)}, manifest is {v}")
...
zipfile="$root/dist/darkharasho-ClearBattlefield-$version.zip"

dotnet build "$root/src/ClearBattlefield/ClearBattlefield.csproj" -c Release

rm -rf "$stage" "$zipfile"
mkdir -p "$stage/plugins"
cp "$manifest" "$root/thunderstore/icon.png" "$root/thunderstore/README.md" "$root/CHANGELOG.md" "$stage/"
cp "$root/src/ClearBattlefield/bin/Release/ClearBattlefield.dll" "$stage/plugins/"
